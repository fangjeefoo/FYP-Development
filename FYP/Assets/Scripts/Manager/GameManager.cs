...
public class GameManager : MonoBehaviour
{
    ...
    void PostData()
    {
        ...
        if (!PlayerPrefs.HasKey("Database Key"))
        {
            key = _database.GetReference(_dbName).Push().Key;
            PlayerPrefs.SetString("Database Key", key);
            _gameData = new GameData(_levelDuration, _selectedExercise);
            _database.GetReference(_dbName).Child(key).Push().SetRawJsonValueAsync(JsonUtility.ToJson(_gameData));
        }
        else
        {
